...
global.daemon_rpc_ws = `wss://${self_hostname}:33310`;
...
function loadConfig(net) {
  try {
    ...
    const config_root_dir =
      'PETROLEUM_ROOT' in process.env
        ? process.env.PETROLEUM_ROOT
        : path.join(os.homedir(), '.petroleum', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 33310); // jshint ignore:line
